...
class mdb {
	...
	public function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) ) {
			return  'Invalid database prefix';
		}
	...
	protected function strip_invalid_text( $data ) {
		...
		if ( $db_check_string ) {
			...
			if ( ! $row ) {
				return 'wpdb_strip_invalid_text_failure';
			}
	...
	public function where($table, $where, $columns = '*', $many = false, $where_format = null) {
		...
		if($columns != '*') {
			...
			$columns_cond = '*';
		$sql = "SELECT $columns_cond FROM `$table` WHERE $conditions";
		if( !$many )
		    $sql .= ' LIMIT 1';

		$this->check_current_query = false;
		
		$query = $this->prepare( $sql, $values );
	...
	public function get_where_cond($table, $where, $where_format = '') {
		$where = $this->process_fields( $table, $where, $where_format );
		if ( false === $where ) {
			return false;
		}
		...
		$conditions = implode( ' AND ', $conditions );
		$result = ["cond" => $conditions, "vals" => $values];
		return $result;
	}
}
?>